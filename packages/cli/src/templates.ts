...
/**
 * The `router` template: file-based routing on the web target. Every file under `src/app/` is a route
 * (`app/index.tsx` → `/`, `app/about.tsx` → `/about`); `mindees dev`/`build` regenerates `src/routes.gen.ts`
 * (a static-import module map) which `createFileRouter` turns into a typed Quantum router — no bundler glob.
 */
const router: Template = {
  name: 'router',
  description: 'File-based routing — screens under src/app/ become routes (Quantum router).',
  files: {
    'package.json': appPackageJson('{{appName}}', { '@mindees/router': PKG_VERSION }),
    // `src/routes.gen.ts` is regenerated by every build → ignore it.
    '.gitignore': `${GITIGNORE}src/routes.gen.ts\n`,
    'tsconfig.json': TSCONFIG,
    'README.md': `# {{appName}}

A MindeesNative app with **file-based routing**. Add a file under \`src/app/\` and it becomes a route:

- \`src/app/index.tsx\` → \`/\`
- \`src/app/about.tsx\` → \`/about\`
- \`src/app/blog/[slug].tsx\` → \`/blog/:slug\` (dynamic), \`[...rest]\` → catch-all, \`_layout\` → layout, \`(group)\` → groupless

\`mindees dev\`/\`build\` regenerate \`src/routes.gen.ts\` (the static-import map \`createFileRouter\` consumes — the browser has no bundler glob). It's git-ignored; just run \`mindees dev\`.
`,
    'src/main.tsx': `import { createDomBackend, render } from '@mindees/renderer'
import { createBrowserHistory, createFileRouter, createRouterView } from '@mindees/router'
import { routes } from './routes.gen.js'

// File-based routing: \`routes\` is generated from src/app/ by \`mindees dev\`/\`build\`.
const router = createFileRouter(routes, { history: createBrowserHistory() })

const root = document.getElementById('app')
if (root) render(() => createRouterView(router), {}, createDomBackend(), root)
`,
    'src/app/index.tsx': `import type { RouteComponentProps } from '@mindees/router'

export default function Home(props: RouteComponentProps) {
  return (
    <view style={{ padding: 24, gap: 12 }}>
      <text style={{ fontSize: 24, fontWeight: 700 }}>Home</text>
      <text>This screen is src/app/index.tsx, served at "/".</text>
      <button onClick={() => props.router.navigate('/about')}>Go to About →</button>
    </view>
  )
}
`,
    'src/app/about.tsx': `import type { RouteComponentProps } from '@mindees/router'

export default function About(props: RouteComponentProps) {
  return (
    <view style={{ padding: 24, gap: 12 }}>
      <text style={{ fontSize: 24, fontWeight: 700 }}>About</text>
      <text>This screen is src/app/about.tsx, served at "/about".</text>
      <button onClick={() => props.router.navigate('/')}>← Back home</button>
    </view>
  )
}
`,
    // Pre-generated so the editor resolves the import on a fresh scaffold; the build keeps it in sync.
    'src/routes.gen.ts': `// AUTO-GENERATED by @mindees/compiler generateRouteModule. Do not edit; regenerate when routes change.
