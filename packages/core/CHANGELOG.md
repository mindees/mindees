...
- a457873: Final pre-1.0 surface cleanup (freeze audit):
...
- e45fcc2: Harden tree-scoped context + overlay visibility after an adversarial review (3 defects, scheduler/
...
- 10523ac: **Tree-scoped context: `provideContext` / `useContext`** (implements ADR-0025, part 1). Alongside the
  explicit `createProvider`, a context value can now flow _implicitly_ down the reactive owner tree: a parent
...
- 9040462: Release-engineering hardening for a 1.0 (roadmap #7):
