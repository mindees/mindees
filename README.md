...
> verified in CI (Phases 8A-8E), and Android now has an embedded QuickJS example
...
> documented experimental scope. Still missing: iOS embedded-runtime parity and
> physical-device proof, so production native mobile apps are not ready.
>
...
- ⏭️ **Phases 8F / 13** — physical-device Android proof, iOS embedded-runtime parity, then examples, benchmarks, docs site & release
...
The repo also has CI-verified iOS/Android host projects and an Android example app
that runs on an emulator through an embedded QuickJS JS<->native bridge. Still
missing: iOS embedded-runtime parity, physical-device proof, app-store packaging,
and the production hardening expected before real mobile apps.
